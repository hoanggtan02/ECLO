<?php
    ...
    //     // Điều kiện WHERE cho truy vấn bảng salaryadvances
    //     $where = [
    //         "AND" => [
    //             "OR" => [
    //                 "sn[~]" => $searchValue, // Tìm kiếm theo mã nhân viên
    //                 "Note[~]" => $searchValue, // Tìm kiếm theo ghi chú
    //             ],
    ...
    //     error_log("WHERE condition for salaryadvances: " . json_encode($where));

    //     // Đếm tổng số bản ghi (không tính LIMIT)
    //     $count = $app->count("salaryadvances", ["AND" => $where['AND']]);
    //     error_log("Total records in salaryadvances: " . $count);

    //     // Lấy dữ liệu từ bảng salaryadvances
    //     $datas = [];
    //     $app->select("salaryadvances", "*", $where, function ($data) use (&$datas, $jatbi, $app) {
    //         // Debug: Kiểm tra dữ liệu lấy được từ bảng salaryadvances
    //         error_log("Record from salaryadvances: " . json_encode($data));

    //         // Lấy tên loại giao dịch từ bảng transactiontypes dựa trên TypeID
    //         $transaction = $app->get("transactiontypes", ["TypeName"], ["TypeID" => $data['TypeID']]);
    //         if (!$transaction) {
    //             error_log("Transaction type not found for TypeID: " . $data['TypeID']);
    //             $transaction_type = $jatbi->lang("Không xác định");
    //         } else {
    //             error_log("Transaction type found for TypeID: " . $data['TypeID'] . ", TypeName: " . $transaction['TypeName']);
    //             $transaction_type = $transaction['TypeName'];
    //         }

    //         // Lấy tên nhân viên từ bảng employee dựa trên sn
    //         $employee = $app->get("employee", ["name"], ["sn" => $data['sn']]);
    //         if (!$employee) {
    //             error_log("Employee not found for sn: " . $data['sn']);
    //             $employee_name = $jatbi->lang("Không xác định");
    //         } else {
    //             error_log("Employee found for sn: " . $data['sn'] . ", name: " . $employee['name']);
    ...
    //             "Amount" => number_format($data['Amount'], 0, '.', ','), // Định dạng số tiền: không có thập phân, dấu phẩy phân cách hàng nghìn
